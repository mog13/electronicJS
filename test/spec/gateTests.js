(function () {
    ...
    describe('test suite for gate functionality', function () {
        ...
        it('should be able to use a string to set and retrieve input pin values', function () {
            gate.addInput(new Wire(0, 'test'));
            gate.setInput('test', 1);
            expect(gate.getInput('test')).toBe(1);
        });

        it('should be able to use a string to set and retrieve output pin values', function () {
        ...
        it('should be able to use a string to set and retrieve input pins', function () {
            var wire = new Wire(0, 'test');
            gate.addInput(wire);
            gate.setInput('test', 1);
            expect(gate.getInputPin('test')).toBe(wire);
            expect(gate.getInputPin(0)).toBe(wire);
        });
        ...
        it('should be able to use a string to set and retrieve output pins', function () {
            ...
            gate.addOutput(wire);
            gate.setOutput('test', 1);
            expect(gate.getOutputPin('test')).toBe(wire);
            expect(gate.getOutputPin(0)).toBe(wire);
        });
    ...
    describe('test suite for basic logic', function () {
        ...
        describe('it should correctly implement generic NAND gates with arbitrary ins/outs', function () {

            it('should work for many inputs one output', function () {
                var gate = new NAND('test', 5, 1);
                expect(gate.getOutput(0)).toBe(1);
                gate.setInput(0, 1);
                gate.setInput(1, 1);
                gate.setInput(2, 1);
            ...
            it('should work for many outputs and two inputs', function () {
                var gate = new NAND('test', 2, 3);
                expect(gate.getOutput(0)).toBe(1);
                gate.setInput(0, 1);
                gate.setInput(1, 1);
                expect(gate.getOutput(0)).toBe(0);
                expect(gate.getOutput(1)).toBe(0);
